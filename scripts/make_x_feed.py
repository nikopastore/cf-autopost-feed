#!/usr/bin/env python3
# rss_x.xml (all) + rss_x_live.xml (latest 1), X-safe truncation + suffix

import xml.etree.ElementTree as ET, re, hashlib, html, sys, os
from email.utils import parsedate_to_datetime
from datetime import datetime, timezone

IN_FEED="rss.xml"; OUT_ALL="rss_x.xml"; OUT_LIVE="rss_x_live.xml"
RESERVED_PREFIX=""; RESERVED_SUFFIX=" #careerforge"; BASE_LIMIT=280
STAMP_RE=re.compile(r"""(?isx)\s*(?:—|–|-|\||:)?\s*(?:\(|\[)?\s*(?:\d{4}[-/]\d{2}[-/]\d{2}(?:[ T]\d{1,2}:\d{2}(?::\d{2})?\s?(?:AM|PM)?)?(?:\s?[A-Z]{2,4})?|\d{1,2}/\d{1,2}/\d{2,4})\s*(?:\)|\])?\s*$""")
def strip_stamp(s): return STAMP_RE.sub("", s or "").strip()
def collapse_ws(s):
    s=re.sub(r"<[^>]+>","",s or ""); s=(s.replace("&nbsp;"," ").replace("&mdash;","—").replace("&#8212;","—").replace("&ndash;","–").replace("&#8211;","–"))
    s=html.unescape(s); return re.sub(r"\s+"," ",s).strip()
ZWJ="\u200d"
...
def emoji_safe_truncate(text, limit):
    ...
    def unsafe_tail(chrs): return chrs.endswith(ZWJ) or (chrs and (is_vs(chrs[-1]) or is_skin(chrs[-1]) or is_keycap(chrs[-1]) or is_regional(chrs[-1])))
...
def smart_text(body):
    ...
    return txt
def load_items():
    if not os.path.exists(IN_FEED): print("rss.xml not found", file=sys.stderr); sys.exit(1)
    t=ET.parse(IN_FEED); ch=t.getroot().find("channel"); items=[]
    for it in ch.findall("item"):
        pub=it.findtext("pubDate") or ""
        try: dt=parsedate_to_datetime(pub); dt=dt if dt.tzinfo else dt.replace(tzinfo=timezone.utc)
        except: dt=datetime.min.replace(tzinfo=timezone.utc)
        items.append((dt,it))
...
def transform(src_items):
    out=[]
    for it in src_items:
        title=strip_stamp(collapse_ws(it.findtext("title") or "")); desc=strip_stamp(collapse_ws(it.findtext("description") or "")); link=(it.findtext("link") or "").strip()
        body=title or desc; text=smart_text(body)
        n=ET.Element("item"); ET.SubElement(n,"title").text=text; ET.SubElement(n,"description").text=text; ET.SubElement(n,"link").text=link
...
def write_feed(path, ch_src, items):
    ...
    ET.ElementTree(root).write(path, encoding="utf-8", xml_declaration=True); print("Wrote", path)
